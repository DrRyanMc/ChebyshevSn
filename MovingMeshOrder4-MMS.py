...
def rhs(t,mu,w, sigmat, sigmas, I, oldval,L):
    hx = L/I
    Q = lambda a,b,t,mu: (0.2*math.sqrt(b-a)*(1+10*t-5*mu*(a+b)),
                          -mu/math.sqrt(3)*math.pow(b-a,1.5),
                          0,0,0)
...
#        bt.append(lambda i,t: right[i] + 1*t*np.max(mu)*right[i]/right[I-1])
    ...
    for i in prange(I):
        ...
        for k in range(K):
            Qtmp = np.array(Q(atval,btval,t,mu[k]))
            right_speed = mu[k] - b
            left_speed = mu[k] - a
            psiR = 0.0
            psiL = 0.0
...
def planeSource(I = 64, L = 1e-1, Nang = 18, tf = 1):
    hx = L/I
    left = np.linspace(-L/2,L/2-hx,I)
    right = np.linspace(-L/2+hx,L/2,I)
    at = []
    ...
    for i in range(I):
        ic[i,:,0] = 1/(300*math.sqrt(right[i]-left[i]))*(-3*left[i]+100*left[i]**3+3*right[i]-100*right[i]**3)
        ic[i,:,1] = -math.pow(right[i]-left[i],1.5)*(right[i]+left[i])/(2*math.sqrt(3))
        ic[i,:,2] = -math.pow(right[i]-left[i],2.5)/(6*math.sqrt(5))
    sol = solve_ivp(ode_rhs, [0, tf], ic.reshape(I*mu.size*5),rtol=1e-11,atol=1e-12, t_eval=[0,tf], method="DOP853")#, method="Radau", vectorized=False)
    phi = np.zeros((I,5))
...
Npoints = 1000
pl = np.zeros((Npoints,2))
pl[:,0] = np.linspace(-1.1,1.1,Npoints)
pl[:,1] = (1.1**2 - pl[:,0]**2) #-(np.abs(pl[:,0]) < 0.1)*(1/100-pl[:,0]**2)
ganapol = interp1d(pl[:,0],pl[:,1], kind="cubic") 
for I in Is:
    L = 1e-1*2
    lt, rt, phi = planeSource(I, L, Nang = 32)
    points, output = make_output(phi, I, lt, rt, pts = 10)
    plt.plot(points, output,'o--')
    plt.plot(pl[:,0], pl[:,1],'k'); 
    plt.show()
    sel = np.argmin(np.abs(0.5*(lt+rt)-pl[50,0]))
    nval = phi[sel,0]/np.sqrt(rt[sel]-lt[sel])+np.exp(-1)/(2+L)
    print(points[points.size//2],output[points.size//2])
    print(ganapol(points[points.size//2]))
    print(output[[points.size//2]] - pl[0,1])
    errs[i] = output[points.size//2]-ganapol(points[points.size//2])
    errRMS[i] = np.sqrt(np.mean((output - ganapol((points)))**2))
